...
for (const recipe of catalog.recipes) {
  ...
  console.log(`${marker} ${recipe.sourcePath} · ${recipe.lineage} v${recipe.version}${recipe.isLatest ? " latest" : ""} · rules ${recipe.ruleEvaluation.status} / v${recipe.ruleEvaluation.rulesetVersion}`);
