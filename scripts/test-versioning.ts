...
const lineage315 = catalog.recipes.filter((recipe) => recipe.lineage === "harfusa-flash-315");
...
const brokenRevision = brokenParent.find((recipe) => recipe.lineage === "harfusa-flash-315" && recipe.version > 1);
...
const baseline = copy(catalog.recipes.find((recipe) => recipe.id === "harfusa-flash-500-v1")) as CatalogRecipe;
const duplicate = copy(baseline);
duplicate.id = "harfusa-flash-500-copy-v1";
duplicate.lineage = "harfusa-flash-500-copy";
